...
from petl.petl_factory import Adapter_Layer, softmax_gating, Linear, LoRA
from petl.petl_factory import PrefixAdapter
...
class RobertaSelfAttention(nn.Module):
    def __init__(self, config, cache_key=None, kernel_lora=None):
        super().__init__()
        ...
        if config.attn_mode == "lora":
            ...
            self.value = Linear(config.hidden_size, self.all_head_size, r=config.attn_bn, lora_alpha=config.lora_alpha,
                                 lora_dropout=config.lora_dropout, lora_init=config.lora_init)
        if config.attn_mode == "inducer" and "Qlora" in config.attn_option:
            self.query = Linear(config.hidden_size, self.all_head_size, r=config.attn_bn, lora_alpha=config.lora_alpha,
                                 lora_dropout=config.lora_dropout, lora_init=config.lora_init)
        ...
        # inducer related
        self.prefix_value_extension = self.config.prefix_value_extension
        ...
        # implement kernel-mix(qvo)
        elif self.attn_mode == 'kernel' and self.config.attn_option == 'QVO':
            self.ef_attn_adapter = kernel_lora
        # implement inducer + value_extension + Q lora
        elif (self.attn_mode == 'inducer' and self.prefix_value_extension):
            self.ef_attn_adapter = PrefixAdapter(config)
    ...
    def transpose_for_scores(self, x):
        """
        The same effect as self._split_heads
        """
    ...
    def forward(
        ...
        if prefix_state is not None and 'prefix' in self.attn_mode:
            ...
            if self.config.attn_option == 'concat':
                ...
                # print(prefix_key.device, key_layer.device)                
        ...
        elif self.config.attn_mode == 'kernel' and self.config.lora_head:
            query_layer, value_layer = self.ef_attn_adapter((hidden_states, key_layer, query_layer, value_layer))
        ...
        # inducer related
        if self.config.attn_mode == "inducer":
            prefix_weights = query_layer + self.ef_attn_adapter((query_layer, None))
            prefix_weights = (query_layer * prefix_weights).sum(dim=3, keepdim=True)
            prefix_weights = prefix_weights / math.sqrt(self.attention_head_size)

            # Additional O(n^2) to compute prefix_weights
            prefix_weights = torch.cat([prefix_weights, attention_scores], dim=3) # bhn'd
            prefix_weights = nn.Softmax(dim=-1)(prefix_weights)
            # attention_scores = nn.Softmax(dim=-1)(attention_scores)
            prefix_weights = prefix_weights[:,:,:,0]
            prefix_weights = self.dropout(prefix_weights)
        ...
        if self.config.attn_mode == "inducer":
            if self.config.prefix_value_extension:
                prefix_output = self.ef_attn_adapter((query_layer, prefix_weights))
                del prefix_weights
            else:
                prefix_output = self.ef_attn_adapter(query_layer)
                prefix_output = torch.einsum("bhn,bhnd->bhnd", prefix_weights, prefix_output)
                attention_probs = attention_probs + prefix_output
                prefix_output = None
                del prefix_weights
...
class RobertaSelfOutput(nn.Module):
    def __init__(self, config, kernel_lora=None):
        super().__init__()
        ...
        elif config.attn_mode == 'kernel' and self.config.attn_option == 'QVO':
            self.ef_attn_adapter = kernel_lora
    ...
    def forward(self, hidden_states, input_tensor, prefix_output=None):
        # keep the MHA output for kernel-mix(*o)
        if self.config.attn_mode == "kernel" and "O" in self.config.attn_option:
            tmp = self._split_heads(hidden_states, self.num_attention_heads,
        ...
        if self.config.attn_mode == "kernel" and "O" in self.config.attn_option:
            _, hidden_states = self.ef_attn_adapter((tmp, None, None, hidden_states))
            del tmp

        # inducer related
        if self.config.prefix_value_extension:
            hidden_states = hidden_states + prefix_output
...
class RobertaAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
        # print(config.lora_head)

        if config.attn_mode == 'kernel' and config.attn_option == 'QVO':
            self.ef_kernel_lora = LoRA(config)
        else:
            self.ef_kernel_lora = None
        self.self = RobertaSelfAttention(config, cache_key='self', 
                                    kernel_lora=self.ef_kernel_lora)
        self.output = RobertaSelfOutput(config, kernel_lora=self.ef_kernel_lora)
        self.pruned_heads = set()

        # inducer related
        self.prefix_value_extension = config.prefix_value_extension
    ...
    def forward(
        ...
        if self.prefix_value_extension:
            attention_output = self.output(self_outputs[0], hidden_states, self_outputs[-1])
            self_outputs = self_outputs[:-1]
        else:
...
class RobertaOutput(nn.Module):
    def __init__(self, config):
        ...
        if config.ffn_mode == 'adapter' and config.ffn_option != 'parallel':
...
class RobertaModel(RobertaPreTrainedModel):
    ...
    def __init__(self, config, add_pooling_layer=True):
        ...
        # print("init", self.embeddings.token_type_ids.device) # on cpu at initialization
    ...
    def forward(
        ...
        # print("token_type_ids++", self.embeddings.token_type_ids.device)
        if token_type_ids is None:
        ...
        #print("position_ids", position_ids) # None, None
        # print("token_type_ids", token_type_ids) # 0, 1
        #print("inputs_embeds", inputs_embeds) # None
        #print("past_key_values_length", past_key_values_length) # int
        # 0, None, 
        ...
        # print("embedding_output.device", embedding_output.device)
...
class RobertaForSequenceClassification(RobertaPreTrainedModel):
    ...
    def forward(
        ...
        # print("token_type_ids++", token_type_ids, self.roberta.device)
